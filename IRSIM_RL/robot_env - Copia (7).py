...
class RobotNavEnv(gym.Env):
    ...
    def reward(self, data):
            ...
            # Center the cone
            center_idx = num_beams // 2 # Index 50
            half_width = cone_width_in_beams // 2 # 5

            front_start = center_idx - half_width # 45
            front_end = center_idx + half_width   # 55
            ...
            # Assuming action is an array like [linear_velocity, angular_velocity]
            angular_velocity = action[1] 

            # Penalty A: Absolute rotation (discourages unnecessary turning)
            rotation_penalty = -self.w_rotation * abs(angular_velocity)

            # Penalty B: Change in action (discourages oscillating/wiggling)
            # We calculate the absolute difference between the current and previous actions
            ...
            elif is_final_stage and dist_ghost <= 0.2:
